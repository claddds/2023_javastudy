...
public class Ex01_Collection {
	/*
		제네릭과 컬렉션(자료구조)
		제네릭: 컬레션이 어떤 객체들로 이루어졌는지를 표시하는 객체(클래스)타입
		컬렉션: 객체들을 모아서 관리하는 인터페이스들을 말한다.
		API: <T> => 객체타입, <E> => 요소(Element) => 컬렉션안에 존재하는 객체 하나를 뜻한다.
			Map형식: <K,V> => key,Value
		...
		iterator(): Iterator<E> => 컬렉션안에 존재하는 객체들을 순서대로 접근하기 위해서
									Iterator 객체를 반환
									요소들을 하나씩 꺼내서 작업할때 사용(for문도 사용 가능)
		remove(Object o): boolean => 인자로 들어온 객체를 삭제, 성공하면 true
		size(): int => 컬렉션안에 존재하는 요소의 수(for문을 사용할 수 있다.)
