	...
	public Ex01_Menu() {
		...
		i_openFile.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				fd.setVisible(true);
				// 실제 불러오는 코딩(I/O)
				String openpath = fd.getDirectory()+fd.getFile();
		...
		i_exitFile.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if(deff.equals(jta.getText())) {
					System.exit(0);
					// 원래는 내용이 변경되면 저장할지를 물어보고 내용이 변경되지 않으면 그냥 종료
				}else {
					//다시 저장 다이알로그
					// 저장, 저장안함, 취소
		...
		i_help.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				// 메세지창 호출
				JOptionPane.showConfirmDialog(getParent(), "도움말");
			}
		});
		i_info.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				// 메세지창 호출
				JOptionPane.showConfirmDialog(getParent(), "간단 메모장 정보");
			}
		});
	}
