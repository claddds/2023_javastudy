...
/*
	...
	자바에서 스레드 생성법
	** 자바에서는 start()를 호출하여 run()가 실행 되면 스레드 실행 됐다고 함
	1. Thread클래스 상속 받기: start(), run() 메서드가 모두 있다.
	2. Runnable 인터페이스 구현: run()만 존재 
	
	Thread클래스 생성자: Thread(), Thread(Runnable 인터페이스를 구현한 객체)
...
public class Ex01_Thread {
	...
	public static void main(String[] args) {
		// 지금은 싱글스레드이다.(즉, 하나의 스레드가 모든 일 다 처리한다.)
		// Thread.currentThread().getName(): 현재 일하는 스레드 이름 출력
		// main 메서드에서 일하는 스레드를 main
