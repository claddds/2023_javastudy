...
public class Ex01_class {
	...
	클래스가 가지고 있는 것: 데이터 = 멤버필드(변수와 상수)
					  기능 = 동작 = 멤버 메서드
					  생성자 = 클래스가 객체로 만들어질때 한번 호출돼서 데이터를 초기화할 수 있는 것
	 				  ** 클래스 이름 = 저장이름 = 파일이름 = 생성자 이름
	 				  
	 클래스 구조: 헤더, 멤버필드, 생성자, 멤버메서드
	 1. 헤더: [접근 제한자] [클래스 종류에대한 예약어] class 클래스 이름 {	}
	 		1) 접근 제한자: 클래스, 멤버필드, 멤버메서드, 생성자 모두 사용할 수 있다.
	 					외부에서 접근할 수 있는 권한의 단계를 말함
	 			가) public: 누구나 다 접근 가능
	 			나) protected: 같은 패키지와 상속관계에서는 접근 가능
	 			다) 생략(default): 같은 패키지에서만 접근 가능
	 			라) private: 외부에서는 접근 불가, 내부멤버(멤버필드, 멤버메서드)끼리만 접근 가능
	 		
	 		2) 클래스 종류에 대한 예약어: 일반적인 클래스 생략한다.
	 			특정클래스에 해당하는 에약어가 존재한다.(final, abstract....)
	 			
	 		3) class: 클래스임을 나타내는 예약어
	 		
	 		4) 클래스 이름: 클래스 이름 = 저장이름 = 파일이름 = 생성자 이름
	 					첫글자 대문자 나머지는 소문자
	 					두 단어 이상일때는 단어 첫 글자들은 대문자
	 ...
	 3. 멤버메서드: 동작, 기능, 실행
	 	- 해당 메서드를 호출(실행)하면 해당 메서드안에 존재하는 내용이 실행됨
	 	...
	 	- 메서드는 다른 메서드를 호출할 수 있음
	 		1) 구성: [접근 제한자][메서드 종류] 반환형 메서드 이름([인자=매개변수]){실행할 내용}
	 			...
	 			나) 메서드 종류: 일반메서드 이부분을 생략한다.
	 						특정 메서드에 해당하는 예약어를 사용(static)
	 			다) **반환형: 해당 메서드가 실행이 끝나면 호출한 곳으로 되돌아 갈 때
	 						함께 가지고 가는 결과의 자료형
	 						만약에 결과를 가지고 가지 않으면 void라는 예약어를 사용한다.
	 						
	 		2) 메서드 이름: 변수와 같음, 첫글자 소문자, 두 단어 이상일때 단지 첫글자들은 대문자
	 					숫자는 중간이나 뒤에 사용, 공백은 사용 불가
	 					
	 		3) **인자 = 매개변수 : 보통은 실행할 때 필요한 정보를 외부에서 받기위한 통로
	 						생략 가능하고 여러개 사용 가능
	 		
	 		4) **오버로딩(중복정의): 메서드 이름이 같으나 인자 수나 인자 자료형이 다르면 다른
	 						메서드로 판단한다.
	 						즉, 인자의 수나 자료형이 다르면 같은 이름의 메서드가 여러개 
	 						존재할 수 있다. 
	 						
	 		5) ** getter() : 호출하는 입장에서 메서드를 이용해서 정보를 가져오는 메서드.
	 		   ** setter() : 호출하는 입장에서 메서드를 이용해서 정보를 변경하는 메서드.
	 		   변수가 private이면 직접 접근을 할 수 없다. => getter()/setter()를 이용해서 접근한다.
	 ...
	 4. 생성자 : **클래스가 객체로 만들어질 때 한번 호출 된다.
	 		...
	 		** 모든 클래스는 반드시 생성자를 가지고 있음
	 		...
	 		3) 클래스를 이용해서 객체 만들때 생성자 호출
	 			...
	 			new 예약어를 사용하면 무조건 heap메모리에 사용영역(저장영역)을 만든다.
