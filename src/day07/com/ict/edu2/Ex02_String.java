	...
	public static void main(String[] args) {
		...
		*/
		...
		5. equalsIgnoreCase(String anotherString):boolean
		 	입력된 문자열과 현재 문자열이 같은면 true, 디르면 false(대소문자를 구별하지 않음)
		 */
		String s5 = "korea";
		String s6 = "KOREA";
		...
		*/
		...
		// 전화번호(뒷부분) 변환 ex)010-9876-****
		/*
		// 내 답
		String p_num = "010-1234-5678";
		int pnum_l = p_num.length();
		res = "";
		
		for(int i = 0; i < pnum_l; i++) {
			char a = p_num.charAt(i);
			if(i>pnum_l-5) {
				res += "*";
			}else {
				res += a;
		...
		*/
		...
		System.out.println("결과: "+res);
		// 전화번호(앞부분) 변환 ex)010-****-7410
		/*
		// 내 답
		res = "";
		for(int i = 0; i < pnum_l; i++) {
			char a = p_num.charAt(i);
			if(i>pnum_l-10 && i<pnum_l-5) {
				res += "*";
			}else {
				res += a;
			}
		...
		*/
		...
		// beginIndex 위치부터 endIndex위치까지(endIndex위치는 포함하지 않는다.)
		...
		//010-1234-5678 => 010-****-5678, 010-1234-****
		...
		*/
		...
		18. toString: String
		 	객체(클래스)에서 toString: 모든 클래스(객체)에서 사용 가능한 메서드
		 							클래스(객체)가 가지고 있는 정보나 값(데이터)들을 문자열로
		 							만들어서 반환하는 메서드
		...
		*/
		...
		20. startsWith(String prefix): boolean 
			...
			startsWith(String prefix, int int toffset): boolean
		...
		*/
		...
		System.out.println(String.valueOf(bol)+1);
